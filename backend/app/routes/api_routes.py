...
# ═══════════════════════════════════════════════════════════════
# SAFE IMPORTS — server won't crash if a service is missing
# ═══════════════════════════════════════════════════════════════
...
try:
    ...
    _rec_service_ok = True
except Exception as e:
    print(f"[WARN] recommendation_service failed to import: {e}")
    _rec_service_ok = False
    catalog_df = pd.DataFrame()
    ALL_CATEGORIES = []
    ALL_SUBCATEGORIES = {}
    PRODUCT_LOOKUP = {}
    def get_product_detail(pid): return None
    def format_product(row): return row
    def recommend_similar_products(sku, top_n=5): return []
    def recommend_by_category(cat, top_n=5): return []
    def recommend_by_brand(q, top_n=10): return {}
    def get_trending(top_n=5): return []
    def get_dynamic_price(pid, uid=None): return None

try:
    from app.services.product_service import handle_prediction
    _prod_service_ok = True
except Exception as e:
    print(f"[WARN] product_service failed to import: {e}")
    _prod_service_ok = False
    def handle_prediction(sku_id): return {"error": "product_service unavailable"}

# ═══════════════════════════════════════════════════════════════
# PRODUCT ENGINE — lazy singleton, won't crash server on failure
# ═══════════════════════════════════════════════════════════════
...
def get_engine():
    ...
    if _engine_error:
        return None
    try:
        from app.models.product_engine import ProductEngine
        _engine = ProductEngine()
        print("[INFO] ProductEngine loaded successfully.")
    except Exception as e:
        _engine_error = str(e)
        print(f"[WARN] ProductEngine failed to load: {e}")
    return _engine

# Pre-load engine at startup (optional — remove if you want fully lazy)
get_engine()
...
def api_health():
    """Simple health check — always returns 200 so the port stays alive."""
    engine = get_engine()
    return jsonify({
        "status": "ok",
        "recommendation_service": _rec_service_ok,
        "product_service": _prod_service_ok,
        "product_engine": engine is not None,
        ...
        categories = [
            {
                ...
                "name": cat,
                "count": len(ALL_SUBCATEGORIES.get(cat, [])),
                "subcategories": [{"code": sub, "name": sub} for sub in ALL_SUBCATEGORIES.get(cat, [])],
            }
            for cat in ALL_CATEGORIES
        ]
...
def api_get_catalog():
    try:
        ...
        return jsonify({
            ...
            "has_more": page < total_pages,
            "categories": [
                {"name": cat, "subcategories": ALL_SUBCATEGORIES.get(cat, [])}
                for cat in ALL_CATEGORIES
            ],
        })
    except Exception as e:
...
def api_get_session(user_id):
    try:
        ...
        for pid in viewed_pids:
            ...
            if product:
                enriched.append(product)
                categories_seen.append(product.get("category", ""))
...
def api_get_recommendations(product_id):
    """
    Multi-strategy recommendations.
    Uses ProductEngine.get_recommendations() as primary source (your Ridge model),
    falls back to recommendation_service if engine is unavailable.
    """
    try:
        user_id = request.args.get("user_id", None)
        sku     = str(product_id)
        ...
        product = PRODUCT_LOOKUP.get(sku)
        if not product:
            # Also try via get_product_detail (handles encoded IDs)
            product = get_product_detail(sku)
        if not product:
            return jsonify({"error": "Product not found"}), 404
        ...
        # ── 1. Similar products via ProductEngine (your ML model) ──
        engine = get_engine()
        if engine is not None:
            try:
                engine_recs = engine.get_recommendations(sku, top_n=5)
                # Normalise keys to match frontend expectation
                similar = [
                    {
                        "product_id":   r.get("sku_id", ""),
                ...
                print(f"[WARN] ProductEngine.get_recommendations failed: {e}")
                similar = recommend_similar_products(sku, top_n=5)
        else:
            similar = recommend_similar_products(sku, top_n=5)

        # ── 2. Same-category products ─────────────────────────────
        cat_recs = recommend_by_category(category, top_n=6)
        cat_recs = [r for r in cat_recs if r.get("product_id") != sku][:5]
        ...
        if user_id and user_id in SESSIONS:
            ...
            for vpid in viewed_pids:
                ...
                if vp:
                    seen_categories.add(vp.get("category", ""))
            for scat in seen_categories:
                if scat and scat != category:
                    recs = recommend_by_category(scat, top_n=2)
                    session_recs.extend(recs)
            session_recs = session_recs[:4]

        # ── 4. Trending ───────────────────────────────────────────
        trending_all  = get_trending(top_n=6)
        trending_recs = [t for t in trending_all if t.get("product_id") != sku][:4]

        return jsonify({
            "product_id": product_id,
            "category_based": {
                "explanation": f"Similar products in {category} > {subcategory} (ML model)",
                "products":    similar,
            },
            "frequently_bought": {
                "explanation": f"Top rated products in {category}",
        ...
        return jsonify({
            "error": str(e),
            "product_id": product_id,
            "category_based":    {"explanation": "Unavailable", "products": []},
            "frequently_bought": {"explanation": "Unavailable", "products": []},
            "session_based":     {"explanation": "Unavailable", "products": []},
            "trending":          {"explanation": "Unavailable", "products": []},
        }), 500


# ═══════════════════════════════════════════════════════════════
# BRAND RECOMMENDATIONS   (GET /api/brand-recommend/<query>)
# ═══════════════════════════════════════════════════════════════
...
def predict():
    """
    Price prediction for a given SKU.
    Tries ProductEngine first (your trained Ridge model),
    then falls back to product_service.handle_prediction.
    """
    try:
        data   = request.get_json()
        sku_id = data.get("sku_id") if data else None

        if not sku_id:
            return jsonify({"error": "sku_id is required"}), 400

        # ── Try ProductEngine directly ────────────────────────────
        engine = get_engine()
        if engine is not None:
            try:
                X, product = engine.preprocess_from_sku(sku_id)
                if X is None:
                    return jsonify({"error": f"SKU '{sku_id}' not found"}), 404

                predicted_price = engine.get_price_prediction(X)

                return jsonify({
                    "sku_id":           sku_id,
                    "product_name":     str(product.get("product_name", "")),
                    "base_price_usd":   float(product.get("base_price_usd", 0)),
                    "predicted_price":  round(predicted_price, 2),
                    "model":            "Ridge Regression",
                })
            except Exception as e:
                print(f"[WARN] ProductEngine prediction failed, falling back: {e}")

        # ── Fallback to product_service ───────────────────────────
        result = handle_prediction(sku_id)
        if "error" in result:
            return jsonify(result), 404
        return jsonify(result)
